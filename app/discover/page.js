...
// 🎯 Podium Step isolated to prevent React from aggressively reloading it
const PodiumStep = ({ league, rank, router }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-end w-[32%] max-w-[160px] group cursor-pointer transition-transform duration-300 hover:-translate-y-2 relative" onClick={() => router.push(`/league/${league.id}`)}>
            <div className={`w-12 h-12 md:w-20 md:h-20 rounded-full bg-black ring-[2px] md:ring-4 overflow-hidden -mb-6 md:-mb-10 z-20 relative flex items-center justify-center shadow-xl group-hover:scale-105 transition-transform ${isFirst ? 'ring-yellow-400' : isSecond ? 'ring-gray-300' : 'ring-amber-600'}`}>
                {league.image_url ? (
            ...
            {/* 🎯 Tailwind classes explicitly written out to prevent JIT compiler misses */}
            <div className={`w-full rounded-t-lg md:rounded-t-xl flex flex-col items-center pt-8 md:pt-14 pb-2 md:pb-3 px-1 md:px-2 relative z-10 
                ${isFirst ? 'h-40 md:h-56 bg-gradient-to-t from-yellow-900/80 to-yellow-600/30 border-t-[3px] md:border-t-4 border-yellow-400 shadow-[0_-15px_40px_rgba(250,204,21,0.25)]' : 
                  isSecond ? 'h-32 md:h-48 bg-gradient-to-t from-gray-800/90 to-gray-600/30 border-t-[3px] md:border-t-4 border-gray-300 shadow-[0_-10px_30px_rgba(209,213,219,0.15)]' : 
                  'h-28 md:h-40 bg-gradient-to-t from-amber-950/80 to-amber-900/30 border-t-[3px] md:border-t-4 border-amber-600 shadow-[0_-10px_30px_rgba(217,119,6,0.15)]'}`}
            >
                <span className={`font-black italic text-lg md:text-4xl leading-none mb-1 drop-shadow-md ${isFirst ? 'text-yellow-400' : isSecond ? 'text-gray-300' : 'text-amber-500'}`}>
...
export default function DiscoverLeaguesPage() {
    ...
    return (
        /* 🎯 FIX 1: Removed flex-col so the sidebar stays on the left! */
        <div className="flex min-h-screen bg-black text-white font-sans selection:bg-pink-500 selection:text-white">
            ...
            <div className={`fixed inset-y-0 left-0 z-[100] w-[80%] max-w-[300px] bg-[#0b0e14] border-r border-gray-800/60 shadow-2xl transform transition-transform duration-300 flex flex-col md:hidden ${showMobileMenu ? 'translate-x-0' : '-translate-x-full'}`}>
                        ...
                        <div className="flex flex-col gap-2">
                            {clientLeagues && clientLeagues.length > 0 ? (
                                clientLeagues.map(league => (
                                    <Link key={league.id} href={`/league/${league.id}`} onClick={() => setShowMobileMenu(false)} className="flex items-center gap-4 p-3 rounded-xl bg-[#12161f] hover:bg-gray-800 border border-gray-800/60 hover:border-pink-500/50 transition-all group">
            ...
            <main className="flex-1 overflow-y-auto pb-24 relative w-full">
                ...
                {/* 🎯 FIX 2: Loading State Wrapped Inside Layout! */}
                {loading ? (
