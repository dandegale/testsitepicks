...
export default function FighterProfile() {
  ...
  // Helper to safely fetch text via proxy
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      // Clean name for search (e.g. "Jon Jones" -> "jones, jon")
      ...
      // ---------------------------------------------------------
      // 1. FETCH IMAGE (TheSportsDB - Strict MMA Filter)
      // ---------------------------------------------------------
      try {
        ...
        if (tsdbData.player && tsdbData.player.length > 0) {
           // Strict Filter: Must be MMA or related to UFC
      ...
      // ---------------------------------------------------------
      // 2. FETCH STATS (UFCSTATS.COM - "THE OFFICIAL API")
      // ---------------------------------------------------------
      try {
          console.log("📊 Connecting to UFCStats Database...");
          
          // Step A: Search for the fighter on UFCStats
          ...
          if (searchHtml) {
              ...
              // Find the row that matches our fighter's first name
              ...
              if (fighterLink) {
                  ...
                  // Step B: Fetch the specific fighter profile page
                  ...
                  // UFCStats puts these in nice, predictable list items
                  ...
                  listItems.forEach(item => {
                      ...
                      if (label === 'Reach') tempBio.reach = value; // Precise Reach!
      ...
      // ---------------------------------------------------------
      // 3. FETCH RANKING (UFC.COM MARKETING SITE)
      // ---------------------------------------------------------
      // UFCStats doesn't show ranking numbers, so we quickly ping UFC.com just for that
      try {
          ...
          if (ufcHtml) {
              ...
              // NEXT FIGHT Check (UFC.com is better for upcoming bookings)
      ...
      // ---------------------------------------------------------
      // 4. FETCH HISTORY (WIKIPEDIA - RELIABLE LIST)
      // ---------------------------------------------------------
      try {
          // Standard Wiki Fetch (Same as before, robust logic)
  ...
  // ... (Render Logic remains largely the same, ensuring Stance is removed from UI)
